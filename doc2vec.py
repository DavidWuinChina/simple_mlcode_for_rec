...
def create_movie_profile(movie_dataset):
    ...
    for i, data in enumerate(movie_dataset.itertuples()):
        ...
        movie_tags = sorted(vector, key=lambda x: x[1], reverse=True)[:50]
        topN_tags_weights = dict(map(lambda x: (dct[x[0]], x[1]), movie_tags))
        # 将类别词的添加进去，并设置权重值为1.0
        for g in genres:
            topN_tags_weights[g] = 0.5
...
# 训练模型并保存
model = Doc2Vec(documents, vector_size=100, window=3, min_count=1, workers=4, epochs=100)
